...
public:
  ...
  void onProcess(Graphics &g) override {
    ...
    if(timer2.elapsedSec() > 4 || timer2.elapsed() < 0) timer2.start();

    if(timer.elapsedSec() > 8 || timer.elapsed() < 0) timer.stop();
    if(timer2.elapsedSec() > 4 || timer2.elapsed() < 0) timer2.stop();
    ...
    if(noteStart && !noteEnd) {
      g.pushMatrix();
      g.translate(1.02, 0.025 * (midiNote - 64), -4);
      g.scale(0.02, 0.02, 1);
      g.color(1, 1, 0.5);
      g.draw(mMesh2);
      g.popMatrix();
    ...
    g.translate(1 - delta1 / 4 - delta2 / 2, 0.025 * (midiNote - 64), -4);
    g.scale(delta1 / 2, 0.02, 1);
    
    g.color(Color(HSV((delta1 / 2 + delta2) / 3, 1, 1)));
    g.draw(mMesh);
...
public:
  ...
  void onCreate() override {
    ...
    //synthManager.synthSequencer().playSequence("galaxy.synthSequence");
